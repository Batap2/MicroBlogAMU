...
public class ServerSocketHandler implements Runnable, ClientSocketHandler {

    private enum Request {PUBLISH, RCV_IDS, RCV_MSG, REPLY, REPUBLISH, CONNECT, SUBSCRIBE, UNSUBSCRIBE, SERVERCONNECT, UNKNOWN};
            ...
            while(true){
                try {
                    ...
                    if (received == null) {
                        ...
                        if(isServerSocket && serverName != null && Server.connectedServers.containsKey(serverName)){
                            Server.connectedServers.remove(serverName);
                            Server.printConnectedServers();
                        }
                        break;
                    } else {
    ...
    private Request recoRequest(){
        ...
        switch(request.toString()){
            ...
            case "SERVERCONNECT":
                return Request.SERVERCONNECT;
            default:
                return Request.UNKNOWN;
        }
    }

    private void requestManager(Request req) throws IOException {
        switch (req){
            case PUBLISH:
                publish();
                break;
            ...
            case SERVERCONNECT:
                ...
                break;
            case UNKNOWN:
                response("ERROR", "Unknown client.request");
    ...
    private void rcv_ids() throws IOException {
        ...
        try{
            ...
            System.out.println("incorrect RCV_IDS client.request");
    ...
    private void rcv_msg() throws IOException {
        int i = 0;
        ...
        if(!checkCommand.toString().equals("RCV_MSG msg_id")){
            System.out.println("incorrect RCV_MSG client.request");
    ...
    private void republish() throws IOException {
    ...
    private void response(String enTete, String body) throws IOException {
        String resp = enTete + "\r\n" + body + "\n";
        socket.getOutputStream().write(resp.getBytes());
    }
    private void response(String enTete) throws IOException {
        String resp = enTete + "\r\n";
        socket.getOutputStream().write(resp.getBytes());
    }

    public String sendToServer(String request) throws IOException {
    ...
    private void serverConnect(){
        if(!socket.getInetAddress().getHostAddress().equals("127.0.0.1")){
            System.out.println("ServerConnect blocking : forbidden address");
            return;
        }

        Pattern p = Pattern.compile("^SERVERCONNECT (\\w+)$");
        Matcher m = p.matcher(received);
        if(!m.matches()){
            System.out.println("SERVERCONNECT syntax error");
            return;
        }

        String serverName = m.group(1);
        if(!Server.connectedServers.containsKey(serverName)){
            Server.connectedServers.put(serverName, this);
            setServerSocket();
            this.serverName = serverName;
            Server.printConnectedServers();
        } else {
            System.out.println("Server " + serverName + " is already connected");
        }
    }

    public void sendServerConnection(String name) throws IOException {
        response("SERVERCONNECT " + name);
    }
